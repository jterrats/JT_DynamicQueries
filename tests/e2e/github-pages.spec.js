...
test.describe('GitHub Pages - Documentation Site', () => {
  ...
  test('Homepage loads successfully', async ({ page }) => {
    ...
    // Verify main heading
    await expect(page.locator('h1').first()).toContainText('JT Dynamic Queries');
  ...
  test('Gallery page loads with videos and screenshots', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /gallery|demos/i }).first()).toBeVisible();

    // Check for video or screenshot containers
    const mediaElements = page.locator('video, img[src*="screenshot"], img[src*="demo"]');
  ...
  test('GitHub Issues documentation is accessible', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/GitHub Issues|User Stories/i);
    await expect(page.locator('body')).toContainText(/v3\.0|version 3/i);
  });

  test('Run As User feature documentation is accessible (no 404)', async ({ page }) => {
  ...
  test('Semantic HTML implementation docs are accessible', async ({ page }) => {
    const response = await page.goto(`${BASE_URL}/SEMANTIC_HTML_FINAL_REPORT.html`);
    expect([200, 304]).toContain(response.status());

    // Verify semantic HTML content
    await expect(page.locator('body')).toContainText(/Semantic HTML|HTML Semántico/i);
  });
  ...
  test('All critical documentation pages return 200 (no 404s)', async ({ page }) => {
    const criticalPages = [
      '/',
      '/gallery.html',
      '/FEATURES_v2.html',
  ...
  test('Footer is present with correct links', async ({ page }) => {
    ...
    const githubLinks = page.locator('a[href*="github.com/jterrats/JT_DynamicQueries"]');
  ...
  test('Deploy to Salesforce button works on homepage', async ({ page }) => {
    ...
    // Find deploy button
    const deployButton = page.locator('a[href*="githubsfdeploy"], img[alt*="Deploy"]').first();
  ...
  test('Links to GitHub issues work', async ({ page }) => {
    ...
    const issueLinks = page.locator('a[href*="github.com/jterrats/JT_DynamicQueries/issues"]');
  ...
  test('Quick Links section contains expected links', async ({ page }) => {
    ...
    const quickLinks = page.locator('text=Quick Links').locator('xpath=following-sibling::*[1]');
