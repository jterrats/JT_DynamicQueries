...
test.describe("Metadata Lifecycle Tests", () => {
  ...
  test("should create configuration and execute query successfully", async ({
    ...
    await page.waitForTimeout(800); // Wait for query validation (increased debounce timeout)

    // Fill Bindings (JSON)
    const bindingsInput = modal.locator(
      'lightning-textarea[data-field="bindings"]'
    );
    ...
    const objectInput = modal.locator(
      'lightning-input[data-field="objectName"]'
    );
  ...
  test("should create configuration with parameters and execute with bindings", async ({
    ...
    await modal
      ...
      .locator("textarea")
      .fill(testQuery);
    await page.waitForTimeout(1000); // Wait for query validation (increased debounce timeout)
    await modal
      .locator('lightning-textarea[data-field="bindings"]')
      .locator("textarea")
      .fill(testBindings);
    await page.waitForTimeout(300);
    await modal
      .locator('lightning-input[data-field="objectName"]')
      .locator("input")
      .fill(testObject);
    await page.waitForTimeout(300);
