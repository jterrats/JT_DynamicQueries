...
function assignPermissionSet() {
    try {
        ...
        const apexScript = path.join(__dirname, '../../../scripts/apex/assign-permset.apex');
        ...
        if (result.includes('Permission Set') && !result.toLowerCase().includes('error')) {
        ...
        console.error('❌ Error executing Permission Set assignment:', error.message);
