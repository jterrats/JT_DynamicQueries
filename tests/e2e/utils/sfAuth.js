...
function getSFSession() {
    try {
        ...
        if (orgInfo.status !== 0) {
            throw new Error('❌ No default org found. Run: sf config set target-org <username>');
        }
...
async function injectSFSession(page, session) {
    ...
    if (isLoginPage) {
        throw new Error('❌ Authentication failed - still on login page. Make sure your SF CLI session is active.');
    }
...
async function getCurrentApp(page) {
    try {
        const appName = await page.locator('one-appnav-brand-logo-text').textContent({ timeout: 5000 });
        return appName.trim();
    } catch {
        return '';
    }
